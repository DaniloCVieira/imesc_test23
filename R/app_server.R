...
req(input$mantel_cutoff)
  ...
  observe({
    ...
    output$style_act<-renderUI({
      req(!is.null(input$sidebarItemExpanded))
      if(input$sidebarItemExpanded=="unsup_expand"){
        tags$style(
          HTML("#sidebar-main > div > li:nth-child(6) > a {
           background: SteelBlue;
           color: white;
      }
      #sidebar-main > div > li:nth-child(7) > a {
           background: #05668D;
           color: white;
      }
    ")
          ...
          tags$style(
            HTML("#sidebar-main > div > li:nth-child(7) > a {
           background: SteelBlue;
           color: white;
      }
      #sidebar-main > div > li:nth-child(6) > a {
           background: #05668D;
           color: white;
      }
    ")
...
if(is.null(menu_steps$anext)&is.null(menu_steps$prev)){
  ...
  observeEvent(input$confirm_newmodelnames,{
   ...
   #
    ...
    #selected<-2:3
    #input<-list()
    #input$editmodel_targ_merge<-"envi_araca"
    #input$merge_model_tab_rows_selected<-selected
    #names(attr(vals$saved_data[['nema_araca']],"rf"))<-"RF model"
  ...
  output$merge_model_tab<-DT::renderDT({
    ...
    DT::datatable(  vals$merge_models   , editable = TRUE, options=list(info = FALSE,lengthMenu = list(c(-1), c("All")), autoWidth=T,dom = 't'))
  })
