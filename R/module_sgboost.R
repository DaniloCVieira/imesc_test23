...
module_server_sgboost <- function (input, output, session,vals,df_colors,newcolhabs ){
  ...
  output$predict_sgboost<-renderUI({
    column(12,style="background: white", uiOutput(ns("sgboost_pred_type")),
           tabsetPanel(id="predsgboost_tab",
                       ...
                       tabPanel(strong("3.3. Resampling predictions"), value="sgboost_tab3_4",
  ...
  get_parts_sgboost<-reactive({
    ...
    partition<- attr(vals$saved_data[[input$data_sgboostY]],"factors")[rownames(vals$saved_data[[input$data_sgboostX]]), input$sgboost_test_partition]
  ...
  getdata_sgboostY<-reactive({
    ...
    if(input$sgboost_type=="Classification"){
      labels <- attr(data,"factors")
      labels<-labels[rownames(vals$saved_data[[input$data_sgboostX]]),input$sgboost_sup]
      labels
    } else{
      data<-data[rownames(vals$saved_data[[input$data_sgboostX]]),input$sgboost_sup]
      data
    }
